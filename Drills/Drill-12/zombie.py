...
class Zombie:
    ...
    def find_ball(self):
        ...
        for i in ball:
            distance = (ball[i].x - self.x) ** 2 + (ball[i].y - self.y) ** 2
            if distance < (PIXEL_PER_METER * 10) ** 2:
                self.dir = math.atan2(ball[i].y - self.y, ball[i].x - self.x)
                return BehaviorTree.SUCCESS
        else:
            self.speed = 0
    ...
    def build_behavior_tree(self):
        ...
        find_ball_node = LeafNode("Find BigBall", self.find_bigball)
        move_to_ball_node = LeafNode("Move to BigBall", self.move_to_ball)
        find_bigball_node = LeafNode("Find Ball", self.find_ball)
        move_to_bigball_node = LeafNode("Move to Ball", self.move_to_ball)
        find_player_node = LeafNode("Find Player", self.find_player)
        move_to_player_node = LeafNode("Move to Player", self.move_to_player)
        chase_node = SequenceNode("Chase")
        chase_node.add_children(find_player_node, move_to_player_node)
        find_ball_seq = SequenceNode("Ball")
        find_ball_seq.add_children(find_ball_node, move_to_ball_node)
        find_bigball_seq = SequenceNode("BigBall")
        find_bigball_seq.add_children(find_bigball_node, move_to_bigball_node)
        wander_chase_node = SelectorNode("WanderChase")
        wander_chase_node.add_children(find_ball_seq, find_bigball_seq, chase_node, wander_node)
